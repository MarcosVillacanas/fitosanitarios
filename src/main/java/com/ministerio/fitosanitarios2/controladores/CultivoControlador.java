...
public class CultivoControlador {
	...
	private void restriccionesProductosSustancias (List<Producto> productos, List <Sustancia> sustancias) {
		...
		Sustancia alguicida = sustancias.get(0);			alguicida.getProductos().add(hidrogeno);
		Sustancia antimicrobiano = sustancias.get(1);		antimicrobiano.getProductos().add(hidrogeno);
		
		Producto helio = productos.get(1);
		Sustancia desecante = sustancias.get(2);			desecante.getProductos().add(helio);
		Sustancia defoliante = sustancias.get(3);			defoliante.getProductos().add(helio);
		
		Producto litio = productos.get(2);
		Sustancia desinfectante = sustancias.get(4);		desinfectante.getProductos().add(litio);
		Sustancia fungicida = sustancias.get(5);			fungicida.getProductos().add(litio);
		
		Producto berilio = productos.get(3);
		Sustancia herbicida = sustancias.get(6);			herbicida.getProductos().add(berilio);
		Sustancia insecticida = sustancias.get(7);			insecticida.getProductos().add(berilio);
		
		Producto boro = productos.get(4);
		Sustancia regulador = sustancias.get(8);			regulador.getProductos().add(boro);
		Sustancia acaricida = sustancias.get(9);			acaricida.getProductos().add(boro);
	}
	...
	private void restriccionesSustanciasPlagas (List<Sustancia> sustancias, List<Plaga> plagas) {
		...
		Plaga pulgon = plagas.get(0);						pulgon.getSustancias().add(alguicida);
		Plaga cochinilla = plagas.get(1);					cochinilla.getSustancias().add(alguicida);
		Plaga trip = plagas.get(2);							trip.getSustancias().add(alguicida);	
		
		Sustancia antimicrobiano = sustancias.get(1);
		Plaga oruga = plagas.get(3);						oruga.getSustancias().add(antimicrobiano);
		Plaga mosca = plagas.get(4);						mosca.getSustancias().add(antimicrobiano);
		Plaga escarabajo = plagas.get(5);					escarabajo.getSustancias().add(antimicrobiano);
		
		Sustancia desecante = sustancias.get(2);
		Plaga saltamontes = plagas.get(6);					saltamontes.getSustancias().add(desecante);
		Plaga gusano = plagas.get(7);						gusano.getSustancias().add(desecante);
		
		Sustancia defoliante = sustancias.get(3);
		Plaga caracol = plagas.get(8);						caracol.getSustancias().add(defoliante);
		Plaga babosa = plagas.get(9);						babosa.getSustancias().add(defoliante);
		
		Sustancia desinfectante = sustancias.get(4);
		Plaga hormiga = plagas.get(10);						hormiga.getSustancias().add(desinfectante);
		Plaga topo = plagas.get(11);						topo.getSustancias().add(desinfectante);
